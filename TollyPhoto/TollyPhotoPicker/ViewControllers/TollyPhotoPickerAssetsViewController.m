//
//  TollyPhotoPickerAssetsViewController.m
//  TollyPhotoLib
//
...
@interface TollyPhotoPickerAssetsViewController () <TollyPhotoPickerCollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDelegate,TollyPhotoPickerBrowserViewControllerDataSource,TollyPhotoPickerBrowserViewControllerDelegate>

// View
// 相片View
@property (nonatomic , strong) TollyPhotoPickerCollectionView *collectionView;
// 底部CollectionView
...
@implementation TollyPhotoPickerAssetsViewController
...
- (void)setSelectPickerAssets:(NSArray *)selectPickerAssets{
    ...
    for (TollyPhotoAssets *assets in selectPickerAssets) {
        if ([assets isKindOfClass:[TollyPhotoAssets class]]) {
...
- (TollyPhotoPickerCollectionView *)collectionView{
    if (!_collectionView) {
        ...
        TollyPhotoPickerCollectionView *collectionView = [[TollyPhotoPickerCollectionView alloc] initWithFrame:CGRectZero collectionViewLayout:layout];
        // 时间置顶
        collectionView.status = TollyPickerCollectionViewShowOrderStatusTimeDesc;
        collectionView.translatesAutoresizingMaskIntoConstraints = NO;
        [collectionView registerClass:[TollyPhotoPickerCollectionViewCell class] forCellWithReuseIdentifier:_cellIdentifier];
        // 底部的View
        [collectionView registerClass:[TollyPhotoPickerFooterCollectionReusableView class]  forSupplementaryViewOfKind:UICollectionElementKindSectionFooter withReuseIdentifier:_footerIdentifier];
...
- (void) setupAssets{
    ...
    [[TollyPhotoPickerDatas defaultPicker] getGroupPhotosWithGroup:self.assetsGroup finished:^(NSArray *assets) {
        [assets enumerateObjectsUsingBlock:^(ALAsset *asset, NSUInteger idx, BOOL *stop) {
            TollyPhotoAssets *zlAsset = [[TollyPhotoAssets alloc] init];
            zlAsset.asset = asset;
...
- (void)setAssetsGroup:(TollyPhotoPickerGroup *)assetsGroup{
...
- (void) pickerCollectionViewDidSelected:(TollyPhotoPickerCollectionView *) pickerCollectionView deleteAsset:(TollyPhotoAssets *)deleteAssets{
    ...
    if (self.selectPickerAssets.count || deleteAssets) {
        TollyPhotoAssets *asset = [pickerCollectionView.lastDataArray lastObject];
        if (deleteAssets){
            asset = deleteAssets;
        }
        ...
        for (NSInteger i = 0; i < self.selectAssets.count; i++) {
            TollyPhotoAssets *photoAsset = self.selectAssets[i];
...
-  (TollyBrowserPhoto *)photoBrowser:(TollyPhotoPickerBrowserViewController *)pickerBrowser photoAtIndexPath:(NSIndexPath *)indexPath{
    TollyBrowserPhoto *photo = [[TollyBrowserPhoto alloc] init];
