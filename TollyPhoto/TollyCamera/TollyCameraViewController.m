...
#import "TollyCameraViewController.h"
#import <AVFoundation/AVFoundation.h>
#import <ImageIO/ImageIO.h>
#import <objc/message.h>
#import "TollyCameraViewController.h"
#import "TollyCameraImageView.h"
#import "TollyCameraView.h"
#import "TollyPhoto.h"

static CGFloat TollyCameraColletionViewW = 80;
static CGFloat TollyCameraColletionViewPadding = 20;
static CGFloat BOTTOM_HEIGHT = 60;

@interface TollyCameraViewController ()<UIActionSheetDelegate,UICollectionViewDataSource,UICollectionViewDelegate,AVCaptureMetadataOutputObjectsDelegate,TollyCameraImageViewDelegate,TollyCameraViewDelegate,TollyPhotoPickerViewControllerDelegate,TollyPhotoPickerBrowserViewControllerDataSource,TollyPhotoPickerBrowserViewControllerDelegate>

@property (weak,nonatomic) TollyCameraView *caramView;
...
// 完成后回调
@property (copy, nonatomic) TollyComplate complate;
...
@implementation TollyCameraViewController
#pragma mark - Getter
#pragma mark Data
...
- (UICollectionView *)collectionView{
    if (!_collectionView) {
        ...
        layout.itemSize = CGSizeMake(TollyCameraColletionViewW, TollyCameraColletionViewW);
        layout.minimumLineSpacing = TollyCameraColletionViewPadding;
        
        CGFloat collectionViewH = TollyCameraColletionViewW;
        CGFloat collectionViewY = self.caramView.height - collectionViewH - 10;
        
        UICollectionView *collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(TollyCameraColletionViewPadding, collectionViewY, self.view.width, collectionViewH)
...
- (void) initialize
{
    ...
    TollyCameraView *caramView = [[TollyCameraView alloc] initWithFrame:CGRectMake(0, 40, self.view.width, self.view.height - 40 - BOTTOM_HEIGHT)];
...
- (UICollectionViewCell *) collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    TollyCamera *camera = self.images[indexPath.item];
    
    TollyCameraImageView *lastView = [cell.contentView.subviews lastObject];
    if(![lastView isKindOfClass:[TollyCameraImageView class]]){
        // 解决重用问题
        UIImage *image = camera.thumbImage;
        TollyCameraImageView *imageView = [[TollyCameraImageView alloc] init];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    TollyPhotoPickerBrowserViewController *browserVc = [[TollyPhotoPickerBrowserViewController alloc] init];
...
- (TollyBrowserPhoto *) photoBrowser:(TollyPhotoPickerBrowserViewController *)pickerBrowser photoAtIndexPath:(NSIndexPath *)indexPath{
    ...
    TollyBrowserPhoto *photo = [TollyBrowserPhoto photoAnyImageObjWith:imageObj];
...
- (void)deleteImageView:(TollyCameraImageView *)imageView{
    ...
    for (TollyCamera *camera in self.images) {
        UIImage *image = camera.thumbImage;
...
-(void)Captureimage
{
    ...
    [self.captureOutput captureStillImageAsynchronouslyFromConnection:videoConnection completionHandler:
     ^(CMSampleBufferRef imageSampleBuffer, NSError *error) {
         ...
         TollyCamera *camera = [[TollyCamera alloc] init];
...
-(void)takePhoto
{
    ...
    {
        TollyCameraViewController *camreaVc = [[TollyCameraViewController alloc] init];
...
-(void)LocalPhoto
{
    TollyPhotoPickerViewController *pickerVc = [[TollyPhotoPickerViewController alloc] init];
    // 最多能选9张图片
    pickerVc.minCount = 9;
