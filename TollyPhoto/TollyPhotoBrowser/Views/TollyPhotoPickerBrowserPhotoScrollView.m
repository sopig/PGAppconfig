//
//  TollyPhotoPickerBrowserPhotoScrollView.m
//  TollyPhotoLib
//
...
@interface TollyPhotoPickerBrowserPhotoScrollView ()<UIActionSheetDelegate> {
    TollyPhotoPickerBrowserPhotoView *_tapView; // for background taps
    TollyPhotoPickerBrowserPhotoImageView *_photoImageView;
}
...
@implementation TollyPhotoPickerBrowserPhotoScrollView

- (DACircularProgressView *)progressView{
    if (!_progressView) {
        DACircularProgressView *progressView = [[DACircularProgressView alloc] init];
        
        progressView.frame = CGRectMake(0, 0, TollyPickerProgressViewW, TollyPickerProgressViewH);
...
- (id)init{
    if ((self = [super init])) {
        ...
        _tapView = [[TollyPhotoPickerBrowserPhotoView alloc] initWithFrame:self.bounds];
        ...
        // Image view
        _photoImageView = [[TollyPhotoPickerBrowserPhotoImageView alloc] initWithFrame:CGRectZero];
...
- (void)setPhoto:(TollyBrowserPhoto *)photo{
    _photo = photo;
    ...
    if (photo.photoURL.absoluteString.length) {
        ...
        if (photoRange.location != NSNotFound){
            [[TollyPhotoPickerDatas defaultPicker] getAssetsPhotoWithURLs:photo.photoURL callBack:^(UIImage *obj) {
