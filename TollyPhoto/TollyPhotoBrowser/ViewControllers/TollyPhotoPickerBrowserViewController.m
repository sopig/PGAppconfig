//
//  TollyPhotoPickerBrowserViewController.m
//  TollyPhotoLib
//
...
#import "TollyPhotoPickerBrowserPhotoScrollView.h"
#import "TollyPhotoCommon.h"
#import "ZLAnimationScrollView.h"

static NSString *_cellIdentifier = @"collectionViewCell";

@interface TollyPhotoPickerBrowserViewController ()<UIScrollViewDelegate,TollyPhotoPickerPhotoScrollViewDelegate,UICollectionViewDelegateFlowLayout,UICollectionViewDataSource,UICollectionViewDelegate>
...
// 数据相关
// 单击时执行销毁的block
@property (nonatomic , copy) TollyPickerBrowserViewControllerTapDisMissBlock disMissBlock;
// 装着所有的图片模型
...
@implementation TollyPhotoPickerBrowserViewController
...
- (UICollectionView *)collectionView{
    if (!_collectionView) {
        ...
        flowLayout.minimumLineSpacing = TollyPickerColletionViewPadding;
        flowLayout.itemSize = self.view.size;
        flowLayout.scrollDirection = UICollectionViewScrollDirectionHorizontal;
        
        UICollectionView *collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 0, self.view.width + TollyPickerColletionViewPadding,self.view.height) collectionViewLayout:flowLayout];
...
- (UILabel *)pageLabel{
    if (!_pageLabel) {
        ...
        NSDictionary *metrics = @{@"ZLPickerPageCtrlH":@(TollyPickerPageCtrlH)};
...
- (void) reloadData{
    ...
    if (self.currentPage >= 0) {
        ...
        if (self.currentPage == [self.dataSource photoBrowser:self numberOfItemsInSection:self.currentIndexPath.section] - 1 && self.currentPage > 0) {
            attachVal = TollyPickerColletionViewPadding;
        }
...
- (UICollectionViewCell *) collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (self.photos.count) {
        ...
        TollyBrowserPhoto *photo = self.photos[indexPath.item]; //[self.dataSource photoBrowser:self photoAtIndex:indexPath.item];
        ...
        TollyPhotoPickerBrowserPhotoScrollView *scrollView =  [[TollyPhotoPickerBrowserPhotoScrollView alloc] init];
        ...
        if (self.currentPage == self.photos.count - 1 && scrollView.x >= 0 && !collectionView.isDragging) {
            ...
            if ([collecitonView2 isMemberOfClass:[UICollectionView class]]) {
                ...
                if (layout.scrollDirection == UICollectionViewScrollDirectionVertical) {
                    scrollView.x = -TollyPickerColletionViewPadding;
                }else{
                }
            }else{
                scrollView.x = 0;
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView{
    ...
    if ((currentPage < [self.dataSource photoBrowser:self numberOfItemsInSection:self.currentIndexPath.section] - 1) || self.photos.count == 1) {
        ...
        tempF.origin.x = -TollyPickerColletionViewPadding;
    }
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView{
    ...
    NSInteger currentPage = (NSInteger)scrollView.contentOffset.x / (scrollView.width - TollyPickerColletionViewPadding);
    
    if (currentPage == self.photos.count - 1 && currentPage != self.currentPage && [[[UIDevice currentDevice] systemVersion] doubleValue] >= 8.0) {
        self.collectionView.contentOffset = CGPointMake(self.collectionView.contentOffset.x + TollyPickerColletionViewPadding, 0);
    }
...
- (void)showHeadPortrait:(UIImageView *)toImageView originUrl:(NSString *)originUrl{
    ...
    [UIView animateWithDuration:.25 animations:^{
        ...
        __weak typeof(TollyPhotoPickerBrowserPhotoScrollView *)weakScrollView = scrollView;
        scrollView.callback = ^(id obj){
